...
% i to j must through some k
The Chapman–Kolmogorov equations. $P_{ij}^{(n)} = \sum_k P_{ik}^{(r)} P_{kj}^{(n-r)}$, forall $0<r<n$.\\
ergodic, for some $n$, $P_{ij}^{(n)}>0,\forall i,j$.\\
% Recall Second law of thermodynamics
% P_{ij}^{(n)} converges, means eventually dispersion.
% below equation is indeed balance
...
% information divide
$H(X,Y) = H(Y) + H_Y(X)$\\
% knowing Y's distribution, might leak information of X.
